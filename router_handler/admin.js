...
exports.updateuser = async (req, res) => {
  ...
  const user = await User.findOneAndUpdate({ _id: req.body._id }, { $set: { username: username, email: email, role: role, state: state } }, { new: true })
  console.log(user);
  if (!user) {
    return res.send({
      status: 400,
...
exports.editclassify = async (req, res) => {
  // console.log(req); 
  const classify = await Classify.findOneAndUpdate({ _id: req.body._id }, { $set: { classify: req.body.classify } }, { new: true });
  if (!classify) {
    return res.send({
      status: 400,
      message:"获取分类信息失败"
...
exports.editarticle = async (req, res) => {
  const { id, title, cover, content, tag, isPublish, description, classify } = req.body;
  // console.log(req.body);
...
exports.changepublish = async (req, res) => {
  console.log(req.body);
  const article = await Article.findOneAndUpdate({ _id: req.body._id }, { $set: { isPublish: req.body.isPublish } }, { new: true })
  if (!article) {
    return res.send({
      status: 400,
      message:'切换失败'
...
exports.changehot = async (req, res) => {
  console.log(req.body);
  const article = await Article.findOneAndUpdate({ _id: req.body._id }, { $set: { isHot: req.body.isHot } }, { new: true })
  if (!article) {
    return res.send({
      status: 400,
      message:'切换失败'
...
exports.updatefriendly = async (req, res) => {
  const friendly = await Friendly.findOneAndUpdate({_id: req.body._id}, {$set: {title: req.body.title, url: req.body.url}}, {new: true});
  if(!friendly){
    return res.send({
      status:400,
      message:'更新友情链接信息失败！'
